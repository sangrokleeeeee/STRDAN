...
class SRMLayer(nn.Module):
    def __init__(self, channel):
        ...
        self.mlp_shared = nn.Sequential(
            nn.Conv2d(3, 512, kernel_size=ks, padding=pw), nn.ReLU(True),
            nn.Conv2d(512, 128, kernel_size=5, padding=2), nn.ReLU(True),
            nn.Conv2d(128, channel, kernel_size=5, padding=2), nn.ReLU(True)
        )
    ...
    def forward(self, x, segmap):
        ...
        segmap = self.mlp_shared(segmap)
        t = self._style_pooling(segmap)
